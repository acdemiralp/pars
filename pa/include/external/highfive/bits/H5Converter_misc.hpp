...
struct data_converter<
    ...
    inline data_converter(std::array<T, S>& arr, DataSpace& space, size_t dim = 0)
    ...
    transform_read(std::array<T, S>& arr) {
        return &(arr[0]);
    }

    inline typename type_of_array<T>::type*
    transform_write(std::array<T, S>& arr) {
        return &(arr[0]);
    }

    inline void process_result(std::array<T, S>& arr) { (void)arr; }

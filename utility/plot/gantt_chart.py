...
for rank_index in range(len(benchmark)):
    for iteration_index in range(len(benchmark[rank_index])):
        offsets[iteration_index] = max(offsets[iteration_index], benchmark[rank_index][iteration_index]["tracing"] + benchmark[rank_index][iteration_index]["communication"]);        
...
for rank_index in range(len(benchmark)):
    ...
    for iteration_index in range(len(benchmark[rank_index])):
        ...
        figure.hbar(y=rank_index, height=0.9, left=offset          , right=offset + tracing                , color="#73C5B6")
        figure.hbar(y=rank_index, height=0.9, left=offset + tracing, right=offset + tracing + communication, color="#FCA556")
